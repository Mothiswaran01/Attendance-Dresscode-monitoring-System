...
How It Works
Audit Bursts — On startup and at each session interval change, the system runs a 200-second high-intensity audit window.
Face Recognition — Every frame is processed through InsightFace. Detected faces are matched against the embedding database using cosine similarity (threshold: 0.50).
Temporal Buffer — A 7-frame sliding window with majority voting (minimum 4 votes) prevents flickering identity changes.
Dress Code Checks — During audit windows, YOLO models detect ID cards and shoes; pose keypoints estimate tuck-in compliance.
Logging — Students meeting the hit threshold (15/300 frames) are logged to Google Sheets per session interval.
Configuration
